...
- **Status:** proposed 2026-07-10 — **for review only.** Not folded into `nightwatch.md`; no FRs
  ...
  behavior for later triage.
- **Motivated by:** dogfooding finding
  [0015 — Patch workflow wording](../dogfooding/0015-patch-workflow-wording.md)
- **Scope:** how `/repo-reconcile` **talks about** a patch it produced for a `derived` artifact. No
  change to what is detected, verified, or written — reconcile still emits a patch file and never
  touches the working tree or checked-out branch.
