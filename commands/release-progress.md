...
- `${CLAUDE_PLUGIN_ROOT}/scripts/release-checks.js` — deterministic hygiene checks (generic source of "done").
...
1. Run the deterministic layer and load config:
   ```
   node ${CLAUDE_PLUGIN_ROOT}/scripts/release-checks.js --repo .
   ```
   ...
   from `${CLAUDE_PLUGIN_ROOT}/templates/RELEASE.md`.
...
6. Write `RELEASE.md`. Emit findings JSON so the brief and ledger see this run:
   - Use the schema in `${CLAUDE_PLUGIN_ROOT}/scripts/lib/findings.js` (require it, or write JSON
