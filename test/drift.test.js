...
module.exports = {
  ...
  'drift: flags a tracked, analyzed, non-allowlisted, undeclared top-level dir': () => {
    const dirs = unclassifiedTopDirs('/x', DEFAULT_CFG, { trackedTop: ['services', 'src', 'docs'] });
  ...
  'drift: an ignore/dev_tooling-excluded dir is never flagged': () => {
    const dirs = unclassifiedTopDirs('/x', DEFAULT_CFG, { trackedTop: ['_bmad', 'node_modules', 'services'] });
  ...
  'drift: a dir named by an authority declaration is classified': () => {
    ...
    const dirs = unclassifiedTopDirs('/x', DEFAULT_CFG, { trackedTop: ['proto', 'services'], authority });
  ...
  'drift: a user dev_tooling extension classifies its dir': () => {
    ...
    const dirs = unclassifiedTopDirs('/x', withExt, { trackedTop: ['agents', 'services'] });
  ...
  'drift: deterministic — multiple undeclared dirs come back sorted': () => {
    const dirs = unclassifiedTopDirs('/x', DEFAULT_CFG, { trackedTop: ['zeta', 'alpha', 'mid'] });
